...
#define SET_CMD19_VAL(name, abnormal, buff, index) \
    do{ \
        buff[index++] = abnormal.AB_ID_JKKZ_##name; \
        buff[index++] = abnormal.AB_JKKZ_##name; \
        LYSprintf(LYSDEBUG,	"<<<<<<<<<< %d==%d	\r\n",abnormal.AB_ID_JKKZ_##name,abnormal.AB_JKKZ_##name);\
    }while(0)

/*	LYSprintf(LYSDEBUG,	"<<<<<<<<<< %d==%d	",abnormal.AB_ID_JKKZ_##name,abnormal.AB_JKKZ_##name);\*/
...
static int do_cmd19 (void *protocol_arg, u8 *buff, u16 len)
{
    ...
    if(buff[0] == VSO_ORGANIZATION)
    {
        ...
        SET_CMD19_VAL(EEPROM, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(TEMPRETURE, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC1V2, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC1V5, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC1V8, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC1V0, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC2V5, jkkz_abnormal, protocol->response.data, index);
        SET_CMD19_VAL(VCC3V3, jkkz_abnormal, protocol->response.data, index);
    }
    protocol->response.is_complete = 1;
...
static int do_cmd2d(void *protocol_arg, u8 *buff, u16 len)
{
    ...
    if (len == 1)
    {
        ...
        case ID_TEMPRETURE:
            ...
            break;
        case ID_VCC1V2:
            voltage = sysmon.vcc1v2;
            status = CMD2D_VOL_STATUS(1) | CMD2D_ONOFF_STATUS(1);
            break;
        case ID_VCC1V5:
            voltage = sysmon.vcc1v5;
            status = CMD2D_VOL_STATUS(1) | CMD2D_ONOFF_STATUS(1);
            break;
        case ID_VCC1V8:
            voltage = sysmon.vcc1v8;
            status = CMD2D_VOL_STATUS(1) | CMD2D_ONOFF_STATUS(1);
            break;
        case ID_VCC1V0:
            voltage = sysmon.vcc1v0_mgtavcc;
            status = CMD2D_VOL_STATUS(1) | CMD2D_ONOFF_STATUS(1);
            break;
        case ID_VCC2V5:
            voltage = sysmon.vcc2v5;
            status = CMD2D_VOL_STATUS(1) | CMD2D_ONOFF_STATUS(1);
            break;
        case ID_VCC3V3:
		...
		LYSprintf(LYSDEBUG,    "<<<<<<<<<< vol==%d ,curr=%d	<<<<<<<<<<<<<<<< \r\n",voltage,current);
        protocol->response.frame_len = index + RESPONSE_HEADER_SIZE;
        /* 发送响应 */
        // TODO:
        ipmi_transmit(protocol);
...
*/
int do_cmd_dispatch(void *protocol_arg, u8 *buff, u8 len)
{
    _protocol_t *protocol = (_protocol_t *)protocol_arg;
    int index = 0, ret = 0;
