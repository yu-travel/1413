...
void timer2_int_init(u16 arr,u16 psc)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=0x3; //抢占优先级1
...
extern uint32_t dev_Open_counter_time[DEVNUM];//单位为ms
//定时器3中断服务函数
void TIM3_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM3,TIM_IT_Update)==SET) //溢出中断
	{
		dev_Open_counter_time[HJJC1_DEV_NUM]++;
		dev_Open_counter_time[HJJC2_DEV_NUM]++;
		dev_Open_counter_time[BF_DEV_NUM]++;
		dev_Open_counter_time[WAOXJ_DEV_NUM]++;
	}
