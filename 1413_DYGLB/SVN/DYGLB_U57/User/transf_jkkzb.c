...
/**************************** Variable Definitions ****************************/
 ...
 DEFAULT_CONFIG_POWER now_default_config_power;//上电默认配置项
 DEFAULT_CONFIG_POWER before_default_config_power;//上电默认配置项

/******************* Macros (Inline Functions) Definitions ********************/
#define DEV_STU_GET()\
	do{	sendmessage.stuAndwarning.bits.hjjc1_stu=   GPIO_ReadOutputDataBit(GPIOA,GPIO_Pin_9);\
		sendmessage.stuAndwarning.bits.hjjc2_stu=   GPIO_ReadOutputDataBit(GPIOA,GPIO_Pin_12);\
		sendmessage.stuAndwarning.bits.bf_stu=    GPIO_ReadOutputDataBit(GPIOB,GPIO_Pin_3);\
		sendmessage.stuAndwarning.bits.waoxj_stu=    GPIO_ReadOutputDataBit(GPIOC,GPIO_Pin_3);\
		sendmessage.stuAndwarning.bits.hjjc1_warning=   !(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_10));\
		sendmessage.stuAndwarning.bits.hjjc2_warning=   !(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_0));\
	    sendmessage.stuAndwarning.bits.bf_warning=    	!(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_4));\
	    sendmessage.stuAndwarning.bits.waoxj_warning=   !(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_4));\
	    sendmessage.stuAndwarning.bits.hjjc1_default=before_default_config_power.bits.hjjc1_default_save;\
		sendmessage.stuAndwarning.bits.hjjc2_default=before_default_config_power.bits.hjjc2_default_save;\
		sendmessage.stuAndwarning.bits.bf_default=before_default_config_power.bits.bf_default_save;\
		sendmessage.stuAndwarning.bits.waoxj_default=before_default_config_power.bits.waoxj_default_save;\
	  }while(0)
/******************* 			Extern	Declare				   ********************/
...
void data_Packet_Creat()
{
	...
	{
	    base = i * 3;
	    sendmessage.messageData[base] = i+MCU4_28V_HJJC1;            			//设备号
...
/******************************************************************************
 * 函 数 名：get_Default_Power
 *
 * 函数说明: 从epprom里，获取默认上电配置
 * 参数说明: 
 * 输入参数: 
 * 输出参数: p_default_config_power：将获取到的信息存入进去，传出
 * 返 回 值: 读取数据是否成功 1：正确，0：错误
 * 
 * 修改记录：
 * --------------------
 *	 
...
{
	uint32_t ret=0;
	ret=I2C_EE_BufferRead(&(p_default_config_power->all),0,1);
	return ret;
	
}

/******************************************************************************
 * 函 数 名：save_Default_Power
 *
 * 函数说明: 将配置下来的默认上电配置存入epprom里
 * 参数说明: 
 * 输入参数: 
 * 输出参数: 
 * 返 回 值: 存入epprom是否成功 1：正确，0：错误
 * 
 * 修改记录：
 * --------------------
 *	 
 ******************************************************************************/
uint32_t save_Default_Power()
{
	uint32_t ret=0;
	now_default_config_power.all=0;
	now_default_config_power.bits.hjjc1_default_save=recivemessage.stuAndwarning.bits.hjjc1_default;
	now_default_config_power.bits.hjjc2_default_save=recivemessage.stuAndwarning.bits.hjjc2_default;
	now_default_config_power.bits.bf_default_save=recivemessage.stuAndwarning.bits.bf_default;
	now_default_config_power.bits.waoxj_default_save=recivemessage.stuAndwarning.bits.waoxj_default;
	//如果本次数据有更改，则将最新数据存入epprom
	if(now_default_config_power.all != before_default_config_power.all)
	{
		ret=I2C_EE_ByteWrite(&now_default_config_power.all,0);
		if(ret==1)//成功存入数据，就更新数据，否则不更新数据，下个循环可以继续存储
		{
			before_default_config_power.all=now_default_config_power.all;
		}
	}
	return ret;
}
