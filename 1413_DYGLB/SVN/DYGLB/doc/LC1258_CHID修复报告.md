...
## 10. 后续问题: 设备物理量换算全零 (2026-08-21, 已上板验证)

### 10.1 现象

CHID 映射修复后原始码正常 (U2 a00(DTJ_V)≈7260023/3.544V 等 16 槽位全 valid),
但 **Device Measured 段 15 路设备 V/I/T 全部为 0**; 而 Auxiliary 段
(轨电压 28.703V/11.949V、HAL、FAULT) 数值全部正确。

### 10.2 根因

`monitor_convert_device()` 中 V/I/T 换算均经 Flash 校准乘加:

```c
physical = s_k[cal_idx + n] * physical + s_b[cal_idx + n];   /* app_monitor.c L166/180/196/215 */
```

- `s_k/s_b` 为静态数组**默认全 0** (app_monitor.c L46-47)
- 唯一初始化入口 `bsp_flash_cal_read(s_k, s_b, CAL_NUM)` 在 `monitor_init()`
  **被注释** (新版板子去掉 Flash, 校准方案废弃)
- 结果: `0 × physical + 0 = 0`, 所有设备物理量被清零
- CH3 辅助路径 (`monitor_convert_aux`) 不乘系数, 故轨电压/HAL/FAULT 不受影响
- 该 bug 在 CHID 修复前即存在, 当时母线未上电、原始码本身≈0, 被 "未供电" 掩盖

### 10.3 修复

`monitor_init()` 显式初始化校准系数: **k=1, b=0 直通** (CAL_NUM=45 组循环);
新版板子无 Flash, `bsp_flash_cal_read` 废弃但 bsp_flash.c/h 保留工程不再调用;
校准框架保留, 后续如需可经协议下发覆盖 (暂不做接口)。

验证判据与结果: 28V 路 V≈28.3V (轨 28.703V 对照)、12V 路 ≈12V 量级、
DYGY/GSDJ 电流非零 mA、温度真实值 —— **上板全部通过**。

### 10.4 附带结案: ÀK/ÀG 显示乱码 (更正此前推断)

- 真因**不是 RTT 读撕裂**: `s_ch3_name[16]` 原仅 14 个初始化值,
  **索引 14/15 为 NULL 指针**, `%s` 打印 NULL 输出垃圾字节 (确定性出现,
  跨上电变化的垃圾内容源于 NULL 解引用位置不定)
- 现场补足 2 个 `"---"` 至 16 项后乱码消失, 最新日志已确认干净
- 此前 §8 "buffer 扩容后待复查" 一项就此关闭

### 10.5 同批入档 (现场调整)

| 项 | 内容 |
|---|---|
| 单位显示 | `s_adc_func`(中文,未引用) → `s_adc_unit = {"V","A","T","Nol"}`, diag 行尾按 ADC 类型打印单位 |
| `DEFAULT_I_LIMIT_MA` | 0→3000 测试值: 0mA 关断式曾致 DYGY/GSDJ 电流异常告警 |
| `diag_init()` | 上电 EFUSE 主动测试注释跳过 |

### 10.6 遗留清单增量更新

- ~~U11 a14/a15 ÀG/ÀK 乱码复查~~ → **已结案** (NULL 指针, 见 10.4)
- 其余 §8 待办不变: 任务周期/看门狗/upload/proto/heartbeat 恢复、
  `power_apply_state(0xFFFF)` 移除、`ADC_REG_DUMP_TEST` 收尾归零、
  `ADC_VREF` 最终校准、`reg_addr_list` 裁剪
