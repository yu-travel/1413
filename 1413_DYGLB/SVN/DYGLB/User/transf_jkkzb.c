...
#define DEV_STU_GET() \
	do{	sendmessage.stuAndwarning.bits.tsgy_stu= GPIO_ReadOutputDataBit(GPIOC,GPIO_Pin_7);\
		sendmessage.stuAndwarning.bits.kf_stu=GPIO_ReadOutputDataBit(GPIOC,GPIO_Pin_9);\
		sendmessage.stuAndwarning.bits.tsgy_warning=!(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_5));\
		sendmessage.stuAndwarning.bits.kf_warning=!(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_8));\
		sendmessage.stuAndwarning.bits.tsgy_default=before_default_config_power.bits.tsgy_default_save;\
		sendmessage.stuAndwarning.bits.kf_default=before_default_config_power.bits.kf_default_save;\
	  }while(0)
...
void data_Packet_Creat()
{
	...
	{
	    base = i * 3;
	    sendmessage.messageData[base] = i;            			//设备号
...
//		}
		...
		{
		DATA_ERROR:
			LYSprintf(LYSDEBUG2,"error, sum=0x%x,recivemessage.sum==%x,\r\n",sum,recivemessage.sum);
			err_cnt++;
		}
...
uint32_t save_Default_Power()
{
	...
	now_default_config_power.bits.tsgy_default_save=recivemessage.stuAndwarning.bits.tsgy_default;
	now_default_config_power.bits.kf_default_save=recivemessage.stuAndwarning.bits.kf_default;
