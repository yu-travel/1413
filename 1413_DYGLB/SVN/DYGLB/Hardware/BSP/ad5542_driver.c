...
// 定义两个 AD5542 芯片的配置
AD5542_ConfigTypeDef ad5542_KF = {
    .cs_gpio_port = GPIOB,
...
AD5542_ConfigTypeDef ad5542_TSGY = {
    .cs_gpio_port = GPIOC,
    .cs_gpio_pin = GPIO_Pin_4,
    .ldac_gpio_port = GPIOC,
    .ldac_gpio_pin = GPIO_Pin_1
};
...
void AD5542_Init()
{
	...
	AD5542_InitCommon();
	// 初始化单个 AD5542 芯片
	AD5542_InitSingle(&ad5542_KF);
	AD5542_InitSingle(&ad5542_TSGY);
}
...
void AD5542Analog_data_Conversion()
{
	...
	for (i = 0;i < DEVNUM; i++)
    {
		...
		{
			case 0://TSGY
				temp = (limit_Current_Data[i]/1000.0f)*0.5;//temp：为4001芯片限流limit_Current_Data，所需要提供的电压，单位V,4001的最大电压只能配置为2.5v，所以最大限流值为2.5/0.5=5A
				limit_Ad5524_Data[TSGY_DEV_NUM]=temp*26214;
				LYSprintf(LYSDEBUG3, "********	limit_Ad5524_Data[0]	=	%d	**************\r\n",limit_Ad5524_Data[i]);
				break;
			case 1://KF
				temp = (limit_Current_Data[i]/1000.0f)*0.125;
				limit_Ad5524_Data[KF_DEV_NUM]=temp*26214;
...
void Ad5542_Send_Data()
{
	...
	switch(temp)
	{
		case 0:			
			//向KF的5542发送数据
			AD5542_WriteData(&ad5542_KF, limit_Ad5524_Data[KF_DEV_NUM]);
			AD5542_TriggerLDAC(&ad5542_KF);
			break;
		case 1:			
			//向TGSY的5542发送数据
			AD5542_WriteData(&ad5542_TSGY, limit_Ad5524_Data[TSGY_DEV_NUM]);
...
//    //向TGSY的5542发送数据
//	AD5542_WriteData(&ad5542_TSGY, limit_Ad5524_Data[0]);
//    AD5542_TriggerLDAC(&ad5542_TSGY);

//	//向KF的5542发送数据
//	AD5542_WriteData(&ad5542_KF, limit_Ad5524_Data[1]);
//    AD5542_TriggerLDAC(&ad5542_KF);
...
//	}
	//向KF的5542发送数据
	AD5542_WriteData(&ad5542_KF, a*2*26214);
	AD5542_TriggerLDAC(&ad5542_KF);
	//向TGSY的5542发送数据
	AD5542_WriteData(&ad5542_TSGY, a*26214);
	AD5542_TriggerLDAC(&ad5542_TSGY);
#endif
}
