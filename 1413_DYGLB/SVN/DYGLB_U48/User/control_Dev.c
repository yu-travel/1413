...
//uint16_t limit_Current_Data[DEVNUM]={1500,1500,1000,500};//限流值，默认值为初始限流值
...
void control_Gpio_Init(void)
{
	...
	// pa10: SFXJ_alert pb0:SFPSD_alert pb4:BQXJ_alert pc9:QGSJ_alert
    ...
    // pa9: SFXJ pa12:SFPSD pb3:BQXJ pc7:QGSJ 
    ...
    // pa11: SFXJ pB1:SFPSD pb5:BQXJ pc6:QGSJ _RESET
...
{	
	...
	stu[SFXJ_DEV_NUM]=recivemessage.stuAndwarning.bits.sfxj_stu;
	stu[SFPSD_DEV_NUM]=recivemessage.stuAndwarning.bits.sfpsd_stu;
	stu[BQXJ_DEV_NUM]=recivemessage.stuAndwarning.bits.bqxj_stu;
	stu[QGSJ_DEV_NUM]=recivemessage.stuAndwarning.bits.qgsj_stu;
	if(stu[SFXJ_DEV_NUM]==0)//sfxj_RESET
	{		
		GPIO_ResetBits(GPIOA, GPIO_Pin_11);
		for(int i=0;i<50;i++);
        GPIO_SetBits(GPIOA, GPIO_Pin_11);
		dev_First_Open_Flag[SFXJ_DEV_NUM]=0;  //设备打开标志清0
		dev_Open_counter_time[SFXJ_DEV_NUM]=0;//设备打开时间清0
	}
	if(stu[SFPSD_DEV_NUM]==0)//sfpsd_RESET
	{
		GPIO_ResetBits(GPIOB, GPIO_Pin_1);
		for(int i=0;i<50;i++);
		GPIO_SetBits(GPIOB, GPIO_Pin_1);
		dev_First_Open_Flag[SFPSD_DEV_NUM]=0;  //设备打开标志清0
		dev_Open_counter_time[SFPSD_DEV_NUM]=0;//设备打开时间清0
	}
	if(stu[BQXJ_DEV_NUM]==0)//bqxj_RESET
	{
		GPIO_ResetBits(GPIOB, GPIO_Pin_5);
		for(int i=0;i<50;i++);
		GPIO_SetBits(GPIOB, GPIO_Pin_5);
		dev_First_Open_Flag[BQXJ_DEV_NUM]=0;  //设备打开标志清0
		dev_Open_counter_time[BQXJ_DEV_NUM]=0;//设备打开时间清0
	}
	if(stu[QGSJ_DEV_NUM]==0)//qgsj_RESET
	{
		GPIO_ResetBits(GPIOC, GPIO_Pin_6);
		for(int i=0;i<50;i++);
		GPIO_SetBits(GPIOC, GPIO_Pin_6);
		dev_First_Open_Flag[QGSJ_DEV_NUM]=0;  //设备打开标志清0
		dev_Open_counter_time[QGSJ_DEV_NUM]=0;//设备打开时间清0
	}
/*4001缓启动配置*/
	if((stu[SFXJ_DEV_NUM]==1)&&(dev_First_Open_Flag[SFXJ_DEV_NUM]==0))//收到打开指令，且还未打开过
	{		
		GPIO_ResetBits(GPIOA, GPIO_Pin_11);		//设置4001自恢复模式					
		dev_First_Open_Flag[SFXJ_DEV_NUM]=1;	//设置为已打开过

	}
	if((stu[SFPSD_DEV_NUM]==1)&&(dev_First_Open_Flag[SFPSD_DEV_NUM]==0))//收到打开指令，且还未打开过
	{
		GPIO_ResetBits(GPIOB, GPIO_Pin_1);
		dev_First_Open_Flag[SFPSD_DEV_NUM]=1;
	}
	if((stu[BQXJ_DEV_NUM]==1)&&(dev_First_Open_Flag[BQXJ_DEV_NUM]==0))//收到打开指令，且还未打开过
	{		
		GPIO_ResetBits(GPIOB, GPIO_Pin_5);		//设置4001自恢复模式					
		dev_First_Open_Flag[BQXJ_DEV_NUM]=1;	//设置为已打开过

	}
	if((stu[QGSJ_DEV_NUM]==1)&&(dev_First_Open_Flag[QGSJ_DEV_NUM]==0))//收到打开指令，且还未打开过
	{
		GPIO_ResetBits(GPIOC, GPIO_Pin_6);
		dev_First_Open_Flag[QGSJ_DEV_NUM]=1;
		selfRecoveTime_0=timer_getms_count();
	}

	OPERATION_SFXJ(recivemessage.stuAndwarning.bits.sfxj_stu);
	OPERATION_SFPSD(recivemessage.stuAndwarning.bits.sfpsd_stu);
	OPERATION_BQXJ (recivemessage.stuAndwarning.bits.bqxj_stu);	
	OPERATION_QGSJ (recivemessage.stuAndwarning.bits.qgsj_stu);	
...
{
	if(dev_Open_counter_time[SFXJ_DEV_NUM]>overTime)
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_11);//设置4001锁存模式	

	}
	if((dev_Open_counter_time[SFPSD_DEV_NUM]>overTime)/*&&(temp1==0)*/)
	{
		GPIO_SetBits(GPIOB, GPIO_Pin_1);//设置4001锁存模式	

	}
	if(dev_Open_counter_time[BQXJ_DEV_NUM]>overTime)
	{
		GPIO_SetBits(GPIOB, GPIO_Pin_5);//设置4001锁存模式	

	}
	if((dev_Open_counter_time[QGSJ_DEV_NUM]>overTime))
	{
...
{
	uint8_t stu[DEVNUM]={0};
	stu[SFXJ_DEV_NUM]=config.bits.sfxj_default_save;
	stu[SFPSD_DEV_NUM]=config.bits.sfpsd_default_save;
	stu[BQXJ_DEV_NUM]=config.bits.bqxj_default_save;
	stu[QGSJ_DEV_NUM]=config.bits.qgsj_default_save;
