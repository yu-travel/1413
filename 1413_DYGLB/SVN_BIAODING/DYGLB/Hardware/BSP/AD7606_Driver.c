...
void AD7606Analog_data_Conversion()
{
	...
	for (i = 0;i < 8; i++)
    {
		...
		{
			case 0:
				analog_data[2] = (adc * 10*1000)*4 / 32768;//KF电
				break;
			case 1:
				temp=(adc * 10*1000) / 32768;
				analog_data[3] = temp /0.125;//KF电流
				break;
			case 2:
				analog_data[0] = (adc * 10*1000)*4 / 32768;//TSGY电压
				break;
			case 3:
				temp=(adc * 10*1000) / 32768;
				analog_data[1] = temp / 0.5;//TSGY电流
				break;				
			default:
...
typedef struct {
    DataPoint kf[2];  // 两组数据值
    DataPoint tsgy[2];  // 两组数据值
} AllNeedData;
...
{
	// 处理kf数据 - 直接用状态作为数组索引
	uint8_t kf_stu = sendmessage.stuAndwarning.bits.kf_stu;
	groupData.kf[kf_stu].x = analog_data[2];
	groupData.kf[kf_stu].y = (kf_stu == TURNON) ? maxTarget : 0;
	
	// 处理tsgy数据 - 直接用状态作为数组索引
	uint8_t tsgy_stu = sendmessage.stuAndwarning.bits.tsgy_stu;
	groupData.tsgy[tsgy_stu].x = analog_data[0];
	groupData.tsgy[tsgy_stu].y = (tsgy_stu == TURNON) ? maxTarget : 0;
}
/**
...
/*打开所有设备开关记录一次电压*/	
	...
	linearFit(groupData.tsgy,2,&k[MCU1_28V_TSGY],&b[MCU1_28V_TSGY]);
//	LYSprintf(LYSDEBUG6, "gongshi : y = %.6f * x + %.6f\n", k[MCU1_28V_TSGY], b[MCU1_28V_TSGY]);
	linearFit(groupData.kf	,2,&k[MCU1_28V_KF],&b[MCU1_28V_KF]);
//	LYSprintf(LYSDEBUG6, "gongshi : y = %.6f * x + %.6f\n", k[MCU1_28V_KF], b[MCU1_28V_KF]);
