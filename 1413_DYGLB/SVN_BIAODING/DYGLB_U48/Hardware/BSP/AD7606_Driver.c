 ...
 static uint16_t data_sliding_average(uint8_t channel,uint16_t new_value)
{
	...
	//LYSprintf(LYSDEBUG, "**before********** AD7606AverageSum[%d] = %d, AD7606ValueBuf[channel][0]= %d  ******\r\n",channel,AD7606AverageSum[channel],AD7606ValueBuf[channel][0]);
	...
	//LYSprintf(LYSDEBUG, "**AFTER********** AD7606AverageSum[%d] = %d, new_value= %d  ******\r\n",channel,AD7606AverageSum[channel],new_value);
...
void AD7606Analog_data_Conversion()
{
	...
	for (i = 0;i < 8; i++)
    {
		...
		{
			case 0:
				temp=(adc * 10*1000) / 32768;					//SFXJ_电流
				analog_data[1] = temp /0.5;
			case 1:
				analog_data[0] = (adc * 10*1000)*4 / 32768; 	//SFXJ_电压
				break;
			case 2:
				temp=(adc * 10*1000) / 32768;					//BQXJ
				analog_data[5] = temp /0.5;
				break;
			case 3:
				analog_data[4] = (adc * 10*1000)*4 / 32768;		//BQXJ
				break;
			case 4:
				temp=(adc * 10*1000) / 32768;					//BF
				analog_data[7] = temp / 0.5;
				break;
			case 5:
				analog_data[6] = (adc * 10*1000)*4 / 32768;		//BF
				break;
			case 6:
				temp=(adc * 10*1000) / 32768;					//SFPSD
				analog_data[3] = temp /0.5;
				break;
			case 7:
				analog_data[2] = (adc * 10*1000)*4 / 32768;		//SFPSD
				break;
			default:
				break;				
		}
...
{
    uint8_t i;
    int16_t adc=0,temp=0;
    for (i = 0;i < 8; i++)
    {
        adc = adc_data[i];
        analog_data[i] = (adc * 10*1000)*4 / 32768;
...
typedef struct {
    DataPoint sfxj[2];  // 两组数据值
    DataPoint sfpsd[2];  // 两组数据值
    DataPoint bqxj[2];  // 两组数据值
    DataPoint qgsj[2];  // 两组数据值
} AllNeedData;
...
{
	// 处理sfxj数据 - 直接用状态作为数组索引
	uint8_t sfxj_stu = sendmessage.stuAndwarning.bits.sfxj_stu;
	groupData.sfxj[sfxj_stu].x = analog_data[0];
	groupData.sfxj[sfxj_stu].y = (sfxj_stu == TURNON) ? maxTarget : 0;
	
	// 处理sfpsd数据 - 直接用状态作为数组索引
	uint8_t sfpsd_stu = sendmessage.stuAndwarning.bits.sfpsd_stu;
	groupData.sfpsd[sfpsd_stu].x = analog_data[2];
	groupData.sfpsd[sfpsd_stu].y = (sfpsd_stu == TURNON) ? maxTarget : 0;
	
	// 处理bqxj数据 - 直接用状态作为数组索引
	uint8_t bqxj_stu = sendmessage.stuAndwarning.bits.bqxj_stu;
	groupData.bqxj[bqxj_stu].x = analog_data[4];
	groupData.bqxj[bqxj_stu].y = (bqxj_stu == TURNON) ? maxTarget : 0;
	
	// 处理qgsj数据 - 直接用状态作为数组索引
	uint8_t qgsj_stu = sendmessage.stuAndwarning.bits.qgsj_stu;
	groupData.qgsj[qgsj_stu].x = analog_data[6];
	groupData.qgsj[qgsj_stu].y = (qgsj_stu == TURNON) ? maxTarget : 0;
}
/**
...
/*打开所有设备开关记录一次电压*/	
	...
	LYSprintf(LYSDEBUG6, "please open BF\r\n");
	...
	linearFit(groupData.sfxj,2,&k[MCU3_28V_SFXJ],&b[MCU3_28V_SFXJ]);

	linearFit(groupData.sfpsd	,2,&k[MCU3_28V_SFPSD],&b[MCU3_28V_SFPSD]);

	linearFit(groupData.bqxj,2,&k[MCU3_28V_BQXJ],&b[MCU3_28V_BQXJ]);

	linearFit(groupData.qgsj	,2,&k[MCU3_28V_QGSJ],&b[MCU3_28V_QGSJ]);
