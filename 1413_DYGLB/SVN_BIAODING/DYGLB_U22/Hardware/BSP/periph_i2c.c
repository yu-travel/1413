...
void i2c1_master_init(void)
{
    ...
    I2C_InitStructure.I2C_OwnAddress1   = i2c1_int.selfaddr<<1;//0x63<<1
...
/*
    @brief      : 外设I2C2做slave初始化 ,lys:本接口做从
    @param[in]  : none
    @param[out] : none
    @retval     : none
*/
void i2c2_master_init(void)
{
    ...
    I2C_InitStructure.I2C_Mode          = I2C_Mode_I2C;
    I2C_InitStructure.I2C_DutyCycle     = I2C_DutyCycle_2;
    I2C_InitStructure.I2C_OwnAddress1   = i2c2_int.selfaddr<<1;  //0x64<<1
    I2C_InitStructure.I2C_Ack           = I2C_Ack_Enable;
    I2C_InitStructure.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit;
    I2C_InitStructure.I2C_ClockSpeed    = I2C_CLOCK_FRQ;
    /* Apply I2C configuration after enabling it */
    ...
    I2C_ITConfig(I2C2, I2C_IT_EVT, ENABLE); //Part of the STM32 I2C driver
    I2C_ITConfig(I2C2, I2C_IT_BUF, ENABLE);
    I2C_ITConfig(I2C2, I2C_IT_ERR, ENABLE); //Part of the STM32 I2C driver
#endif
    delay_ms(1);
#endif
}
...
/*
    @brief      : 外设I2C3做master初始化,lys:本接口做主
    @param[in]  : none
    @param[out] : none
    @retval     : none
*/
...
void i2c_all_init(void)
{
    ...
    i2c2_master_init();
    delay_ms(100);
    i2c1_master_init();      // I2C1作为主机,与接口扩展板通信,用于测试
    delay_ms(100);
    i2c3_master_init();
...
uint8_t gpioi2c_read_bytes(_i2c_interrpt_t* handler, u8 slaveaddr)
{
    ...
    iic_start(i2cbus);                /* 发送起始信号 */
    iic_send_byte(i2cbus, (slaveaddr<<1)|1);      /* 发送设备地址 */
    iic_wait_ack(i2cbus);             /* 每次发送完一个字节,都要等待ACK */
    
    handler->status = I2C_RX_START;
    
    temp = iic_read_byte(i2cbus, ack);  /* 发送低位地址 */
    u8_ring_buffer_queue(&handler->rb_handler, temp);
    frame_len = iic_read_byte(i2cbus, ack);  /* 发送低位地址 */
    u8_ring_buffer_queue(&handler->rb_handler, frame_len);
...
/*!
    ...
    @retval     : none
*/
void i2c_master_event_irq_handler(_i2c_interrpt_t *handler)
{
    u8 data = 0;
    u32 event = 0;
#if 1
    event = I2C_GetLastEvent(handler->dev);
    //if(handler->mode == I2C_MODE_MASTER)
    //{
        switch(event)
        {
            /*--------------- master transmit ---------------------*/
           ...
           case I2C_EVENT_MASTER_BYTE_RECEIVED:
               ...
               handler->rxindex++;
               if(handler->rxindex == 2) // 获取内容段长度
               {
                   handler->rxcount = data;
               }
               if(handler->rxindex == (handler->rxcount+2-1))
               {
                   I2C_AcknowledgeConfig(handler->dev, DISABLE);   // 最后一字节NACK
               }
               if(handler->rxindex == (handler->rxcount+2))
               {
                   handler->rxindex = 0;
                   handler->status = I2C_RX_DONE;
                   I2C_GenerateSTOP(handler->dev, ENABLE); // 产生STOP
   ...
   //}
    ...
    //if(handler->mode == I2C_MODE_SLAVE)
    //{
        //switch(event)
        //{
        //}
...
void I2C1_EV_IRQHandler(void)
{
    ...
    //i2c_slave_event_irq_handler(&i2c1_int);
}
...
void I2C1_ER_IRQHandler(void)
{
    ...
    /* Check on I2C1 AF flag and clear it */ 
    if (I2C_GetITStatus(I2C1, I2C_IT_ARLO))   //仲裁出错
    {
...
#endif
    i2c_master_send_bytes(i2cptr, IPMI_PWR_I2CA_ID, "12345678", 8); // 电源板I2CA地址
