...
void AD7606Analog_data_Conversion()
{
	...
	for (i = 0;i < 8; i++)
    {
		...
		{
			case 0:
				analog_data[7] = (adc * 10*1000) / 32768; 	//HALL_GSDJ2
				break;
			case 1:
				analog_data[6]=(adc * 10*1000) / 32768;		//HALL_GSDJ1
				break;
			case 2:
				analog_data[0] = (adc * 10*1000)*2/ 32768;	//GSDJ1
				break;
			case 3:
				temp=(adc * 10*1000) / 32768;				//GSDJ1
				analog_data[1] = temp / 0.5;
				break;
			case 4:
				analog_data[4] = (adc * 10*1000)*2 / 32768;	//DYGY
				break;
			case 5:
				temp=(adc * 10*1000) / 32768;				//DYGY
				analog_data[5] = temp /0.5;
				break;
			case 6:
				analog_data[2] = (adc * 10*1000)*2/ 32768;	//GSDJ2
				break;
			case 7:
				temp=(adc * 10*1000) / 32768;				//GSDJ2
...
void AD7606_Display(void)
{
    ...
    {
        adc = adc_data[i];
        analog_data[i] = (adc * 10*1000)*2 / 32768;
...
typedef struct {
    DataPoint gsdj1[2];  // 两组数据值
    DataPoint gsdj2[2];  // 两组数据值
	DataPoint dygy[2];  // 两组数据值
} AllNeedData;
...
{
	// 处理gsdj1数据 - 直接用状态作为数组索引
	uint8_t gsdj1_stu = sendmessage.stuAndwarning.bits.gsdj1_stu;
	groupData.gsdj1[gsdj1_stu].x = analog_data[0];
	groupData.gsdj1[gsdj1_stu].y = (gsdj1_stu == TURNON) ? maxTarget : 0;
	
	// 处理gsdj2数据 - 直接用状态作为数组索引
	uint8_t gsdj2_stu = sendmessage.stuAndwarning.bits.gsdj2_stu;
	groupData.gsdj2[gsdj2_stu].x = analog_data[2];
	groupData.gsdj2[gsdj2_stu].y = (gsdj2_stu == TURNON) ? maxTarget : 0;
	// 处理dygy数据 - 直接用状态作为数组索引
	uint8_t dygy_stu = sendmessage.stuAndwarning.bits.dygy_stu;
	groupData.dygy[dygy_stu].x = analog_data[4];
	groupData.dygy[dygy_stu].y = (dygy_stu == TURNON) ? maxTarget : 0;
}
/**
...
/*打开所有设备开关记录一次电压*/	
	...
	linearFit(groupData.gsdj1,2,&k[MCU2_12V_GSDJ1],&b[MCU2_12V_GSDJ1]);
	linearFit(groupData.gsdj2	,2,&k[MCU2_12V_GSDJ2],&b[MCU2_12V_GSDJ2]);
	linearFit(groupData.dygy	,2,&k[MCU2_12V_DYGY],&b[MCU2_12V_DYGY]);
