...
/*
    @brief      : 系统启动后1s初始化i2c
*/
_time_t sysi2c_timer = {0};
void system_i2cinit_callback(_time_t* timer, void* userData)
{
    i2c_all_init();
}
void system_i2cinit_timer_start(void)
{
    softtimer_start(&sysi2c_timer, 1000, system_i2cinit_callback, &sysi2c_timer);
}
...
int softtimer_init(void)
{
    ...
    /* 添加softtimet start */
    blink_timer_start();
    //system_reset_timer_start();
    system_i2cinit_timer_start();
